...
public class FPMovementScript : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            // make the player
            playerController.Move(Vector3.forward * moveSpeed * Time.deltaTime);
        }
        */
        if (direction.magnitude >= 0.1f)
        {
            ...
            float targetAngle = Mathf.Atan2(direction.x, direction.z) * Mathf.Rad2Deg + playerController.transform.eulerAngles.y;
